    ...
    return (
        <div className="md:basis-[25%] relative md:z-0 z-10 bottom-0 md:h-full pb-4 md:pb-0">
            <CardWrapper className="w-full md:h-full md:shadow-xl md:p-6 md:rounded-xl rounded-none shadow-none p-1 md:bg-white bg-white/10">
                <button onClick={toogleSideBar} data-drawer-target="default-sidebar" data-drawer-toggle="default-sidebar" aria-controls="default-sidebar" type="button" className="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none ring-2 ring-gray-400 cursor-pointer">
                    <WrapperIcon Element={Menu} />
                </button>

                <div ref={asideBar} className="absolute md:right-0 md:top-0 right-2/5  left-0 top-[60px] z-40 bottom-0 -translate-x-full md:translate-x-0 transition-all duration-200 flex flex-col justify-between h-screen" aria-label="Sidebar">
                    <div className="h-full px-3 py-4 overflow-y-auto flex flex-col gap-y-2">
                        ...
                        <ul className="font-medium flex flex-col gap-y-2">
                            <li>
                                <NavLink onClick={toogleSideBar} to={"/"} className={(data) => {
                                    const { isActive } = data;
                            ...
                            <li>
                                <NavLink onClick={toogleSideBar} to={"/leaderboard"} className={(data) => {
                                    const { isActive } = data;
                            ...
                            <li>
                                <NavLink onClick={toogleSideBar} to={"/students"} className={(data) => {
                                    const { isActive } = data;
                            ...
                            <li>
                                <NavLink onClick={toogleSideBar} to={"/analytics"} className={(data) => {
                                    const { isActive } = data;
                            ...
                            <li>
                                <NavLink onClick={toogleSideBar} to={"/settings"} className={(data) => {
                                    const { isActive } = data;
                    ...
                    <div className="h-full px-3 py-4 overflow-y-auto flex flex-col gap-y-2 md:justify-end justify-start items-start lg:items-end">
